...
$$
\hat{V}(\hat{T}_y) = \frac{1}{n(n - 1)} \sum_{j=1}^{3} \left( \frac{y_{i_j}}{p_{i_j}} - \hat{T}_y \right)^2 \approx 759{,}188.4
$$
...
The above R was used for validation, and for this problem in particular the estimates produced using R are similar but do not exactly match the hand calculations. 
...
$$
\hat{SE}(\hat{T}_y) = \sqrt{759{,}188.4} \approx 871.3142
$$

Taken together, our 95% confidence interval is of the form: 

$$
\text{CI}_{95\%} = \hat{T}_y \pm 1.96 \cdot \hat{SE} \approx (40{,}808.22,\ 44{,}223.78)
$$
...
Note: Again, we get a slightly different answer when explicitly calculating this without any rounding in intermediate steps. The resulting confidence interval would be: $(40{,}814.6,\ 44{,}217.4)$
...
$$
\hat{SE}(\hat{\bar{Y}}) = \frac{\hat{SE}(\hat{T}_y)}{N} = \frac{871.3142}{145} \approx 6.01
$$

Explicitly, without rounding (hand calculation), we have: The average number of patients discharged per hospital is ${293.34}$ with Standard Error 6.01.

Via R with less precision, we have: The average number of patients discharged per hospital is ${293.34}$ with Standard Error 5.99.
...
To that end, note that for any household $i$:  

$$
\pi_i^{(1)} = \frac{n_1}{N} = \frac{200}{5000} = 0.04
$$

Given Phase 1 sample, the conditional inclusion probability is:  
...
We can then return to the original expression for the $\pi^*$ estimator:  

$$
\hat{Y}^* = \sum_{i \in A_2} \frac{y_i}{\pi_i^*} = \frac{1}{0.016} \sum_{i \in A_2} y_i
$$

Using the Phase 2 sample mean, $\bar{y}_2 = 58$), we calculate $\sum_{i \in A_2} y_i$ by:  
...
Dividing by the total population size $N = 5,000$, taken from Phase 1:  
...
Although the $\pi^\star$-estimator is exactly defined in the lecture slides, Slide 10 of Ch11p1, we do not have the exact unit-level information required for exact calculation, i.e., we do need the individual $y_i$ values and it is not sufficient to use the average like we did for the point estimate in the prior question. For that reason, we approximate its variance by deconstructing the variance formula into its Phase 1 and Phase 2 components. 
...
The total variance then is given by: 

$$
5000^2 \cdot (0.0048 + 0.0075)* 100 = 30,750,000
$$
...
is the approximate Phase 1 variance, 

$$
\left(\frac{1}{r} - \frac{1}{n}\right) S_{ee}
$$ 

is the approximate Phase 2 variance, 
...
We may simplify the expression above somewhat, to get: 
