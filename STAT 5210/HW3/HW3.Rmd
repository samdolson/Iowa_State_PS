...
# Outline: 
  - Q1: G2G
  - Q2: G2G 
  - Q3: Check variance comparision b/w the two estimators (approximate formula is correct?) 
  - Q4: Bit iffy around parts 1-3 
...
Note: When I manually calculated this (greater precision) the estimate came out to 42,517.87, which is close but slightly different than the above. This also has implications for the resulting confidence interval, though only slightly. 
...
```{r}
varianceEst <- (1/(n*(n-1))) * sum(((y_ij/pi_ij) - tHat)^2)
varianceEst
```
...
Note: Should this be $\times 10^{3}$ instead, why minus in the table?
...
Giving us the ratio $15.26 \times 10^{\pm{3}}$ 

Via linearizaton, we define the variable z as:
...
Giving us an estimated per capita income of ${15.26}$ with estimated variance ${0.752}$. 

Note: The answer here is in units of "income per person", which I believe would be something like "\$$\times 10^{\pm{3}}$/person", though that again depends on the exact units of dollars here. 
...
$$
58 + (3.2 - 3.5) \times 4.045 \approx {56.79}
$$
...
The regression estimator is more flexible, efficient, and robust. In that order: The regression estimator can incorporate multiple covariates, which can improve the generalizability of results. Also, because the regression estimator takes advantage of covariates, it exploits correlation structure more effectively, and as a result can be more efficient (have smaller variance) when the covariate(s) are strongly correlated with the response. Lastly, the regression estimator is more robust in the sense that it is less sensitive to violations of the ratio model assumptions, i.e., it can still be consistent when the model is mis-specified.  

Note: It just so happens that the estimates and approximate variances in the above are equal, which occurs because the regression reduces to (is functionally equivalent to) the ratio estimator when the intercept is zero. 
...
No, this is not a generally good estimate! This estimate is problematic because it assumes treatments were randomly assigned, which is not the case! This introduces bias and is possibly being confounded by other factors that may be relevant to account for such as age, gender, or other factors.

At least that one doesn't have any discrepancy with the R validation! 
...
The AIPW estimator is called "doubly robust" because the AIPW estimator is consistent if either the OM or the PM is correct. Specifically: The AIPW model can still be consistent if either the OM or PM are mis-specified. This is not XOR though, at least one needs to be correctly specified! 