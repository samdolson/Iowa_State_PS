...
The Negative Binomial distribution with parameters $r = 3$ and $p$ has a mean:
...
The Double Exponential distribution has a mean $\mu$ and variance $2\sigma^2$. 

Given a random sample $X_1, X_2, \ldots, X_n$, the sample mean and sample variance are:
...
And overall MLE is not unique, i.e., it is defined as a function of x (the MLE depends on the value of x), as illustrated above. 
...
To find the maximum, we differentiate $\ell(\theta)$ with respect to $\theta$ and solve for setting this relation equal to 0:

$$
\frac{\partial \ell(\theta)}{\partial \theta} = -\frac{n}{\theta} + \frac{\sum_{i=1}^n x_i}{\theta^2} -\frac{n}{\theta} + \frac{\sum_{i=1}^n x_i}{\theta^2} = 0
$$
