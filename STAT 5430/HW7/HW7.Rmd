...
This density under the alternative hypothesis is greater than 1 when $x < \frac{1}{2}$, and less than 1 when $x > \frac{1}{2}$. So $f(x \mid \theta_1)$ has more mass in the tails (0 and 1) than under $H_0$. So we'd expect the likelihood ratio statistic to be much larger (and the test function will tend to reject $H_0$) for observations near the tails of the distribution (near 0 or near 1).  

So, generally, the most powerful test at level $\alpha$ will reject $H_0$ for values of $x$ far from $1/2$, i.e. when values are "close" to 0 or "close" to 1. We want to, and will, calculate that exact level of "closeness" based on the size of the test. 
