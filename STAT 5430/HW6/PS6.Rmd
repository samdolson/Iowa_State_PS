...
Since $\theta_2 - \theta_1 > 0$ and $x \in (0,1)$, the function $x^{\theta_2 - \theta_1}$ is increasing in $x$, which in turn means the likelihood ratio $\Lambda(x)$ is increasing in $x$, which in turn means the family has a monotone likelihood ratio in $x$.
