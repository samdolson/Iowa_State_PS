...
(b): $Y = 1 - X^2 \rightarrow X = \sqrt{1 - Y}$, and $-1 < x < 1 \rightarrow 0 < y < \sqrt{2}$
...
$$f_Y(y) = 
\begin{cases}
  \frac{3}{8}(\sqrt{1 - y})^2 &  0 < y \leq 1\\
  \frac{3}{8}(1 - y)^2 & 1 < y < \sqrt{2}
\end{cases}$$
...
Let the random variable Y be defined as $Y = u(X) = F_x(x)$
...
$F_x(X) = \int{f(x)dx} = \int \frac{x-1}{2} = \frac{x^2 - 2x}{4}$
