...
Recall also that we know the Covariance Matrix as $Y | X = x \sim N(x, 1)$, meaning Cov(X, Y) = 1 = Cov(Y, X). Taking this into account then: 
...
We may also find the distribution of $V \mid W = w$ directly by calculating the closed form of its pdf. The following does so for $X \mid Y = y$, which we know to be $V \mid W = w$ given equivalence of the random variable X $\equiv$ V, Y $\equiv$ W. To that end: 
...
For the top part of the equation, note X and Y are independent, such that: 

$$
P(X = x, X + Y = k)  = P(X = x, Y  = k - x) = P(X = x) P(Y = k - x)
$$
...
Again, given X and Y are independent, we evaluate the top as:

$$
P(X = x, Y  = k - X) = P(X = 1, Y = k-1) + P(X = 2, Y = k-2) + ... + P(X = k-1, Y = 1)
$$

$$
P(X = x, Y  = k - X) = (1 - p)^{k - 2} p^2 + ... + (1 - p)^{k - 2} p^2 = \sum_{x=1}^{k-1} P(X =x, Y  = k - x)
$$

Then we evaluate the bottom of the initial formula: 

$$
P(X = x, Y  = k - X) = \sum_{x=1}^{k-1} P(X =x, Y  = k - x) = \sum_{x=1}^{k-1} (1 - p)^{k - 2} p^2 = (k - 1) (1 - p)^{k - 2} p^2
$$

Returning the overall formula, we evaluate as: 

$$
P(X = x | Y  = k - X) = \frac{P(X = x,  Y  = k - X)}{P(Y  = k - X)} = \frac{(1 - p)^{k - 2} p^2}{(k - 1) (1 - p)^{k - 2} p^2} = \frac{1}{k - 1}
$$

For x = 1, 2, ..., k - 1, y = 1, 2, ..., k - 1
