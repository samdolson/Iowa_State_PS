...
Define an infinite sequence of disjoint events, $A_1, A_2, ... \in \mathbb{B}$ as pairwise disjoint sets.  

Furthermore, let: 
$(\bigcup\limits_{i=1}^{\infty}{A_i}) = (\bigcup\limits_{i=1}^{n}{B_i})$
such that their associated probabilities are: 
...
$\sum\limits_{i=1}^{\infty}{P(A_i)} = \sum\limits_{i=1}^{n}{P(B_i)} + \sum\limits_{i=n+1}^{\infty}{P(A_i)} = \sum\limits_{i=1}^{n}{P(B_i)} + \sum\limits_{i=n+1}^{\infty} \emptyset$
...
Notes: 
https://stats.stackexchange.com/questions/25238/how-can-i-prove-that-the-cumulative-distribution-function-is-right-continuous
https://math.stackexchange.com/questions/480042/prove-that-the-cdf-of-a-random-variable-is-always-right-continuous
https://math.stackexchange.com/questions/234292/continuity-from-below-and-above


$\downarrow$ means a decrease or a negative change in a value or function, or "approaching from the right" 
...
Objective: The cdf $F$ is right continuous at some point a iff $\forall n \geq 1, {\{ x_n\}}$, such that $x_n \downarrow a$, gives $F(x_n) \downarrow F(a)$
...
One Direction: If $X(\omega) \leq x_n \space \forall n \geq 1$ since $x_n \downarrow a$, $X(\omega) \leq a$

Other Direction: If $X(\omega) \leq a$, as $a \leq x_n \space \forall n \geq 1$, giving us $X(\omega) \leq x_n \space\forall n \geq 1$

End Result: 

$$F(X_n) = P(X \leq x_n) = P(A_n) \downarrow P(\bigcap\limits_{i=1}^{\infty}A_i)$$

However, as 

$$P(\bigcap\limits_{i=1}^{\infty}A_i) = P(X \leq a) = F(a)$$

We may conclude: $F(x_n) \downarrow F(a)$
