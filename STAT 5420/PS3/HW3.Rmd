...
# Overview 
  - 1: DONE
  - 2: DONE
  - 3: Part (b) tidy integration-by-parts, (c) left TO-DO
  - 4: DONE
  - 5: Part (b) TO-DO 
  - 6: DONE
...
$$\int\limits_{-\infty}^{\alpha}{e^{tx} \frac{1}{2\beta}e^{\frac{(x-\alpha)}{\beta}} dx} =$$ 

> 2. 

$$\int\limits_{\alpha}^{\infty}{e^{tx} \frac{1}{2\beta}e^{\frac{-(x-\alpha)}{\beta}} dx} =$$ 
...
$$M_Y(t) = \int_{-\infty}^{\infty} e^{te^x} f_X(x) dx = f_X(x) = \int_{-\infty}^{\infty} e^{te^x} \frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}} dx$$

For any x, we may break the above integral into the sum of two integrals, specifically: 

$$M_Y(t) = \int_{-\infty}^{x} e^{te^x} \frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}} dx + \int_{x}^{\infty} e^{te^x} \frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}} dx$$

$$x \rightarrow \infty$$ 
$$e^x \to \infty$$ and
$$e^{te^x} \to \infty$$ for $$t > 0$$
...
$$M_Y(t) = \int_{-\infty}^{\infty} e^{te^x} \frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}} dx$$

As $$x \to \infty$$
The term $$e^{te^x}$$ 
grows much faster than $$e^{-\frac{x^2}{2}}$$ decays, so:
...
$$M_Y(t) = \int_{-\infty}^{\infty} e^{te^x} \frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}} dx$$ 
Only exists for $$t \leq 0$$, or stated differently, the MGF does not exist for positive t! 
