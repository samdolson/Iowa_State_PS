...
By unraveling the integral we denote an exact term within the summation. 
...
Let $z = y^\gamma / \beta \rightarrow dy = \beta / \gamma dz$

Change of variables allows us to rewrite the above as: 

$$EY^n = \beta^{n / \gamma}  \int\limits_{z = 0}^{\infty} z^{n/\gamma} e^{-z} dz$$ 

Recognizing this as the $\Gamma (n / \gamma + 1)$ function we thus have: 
