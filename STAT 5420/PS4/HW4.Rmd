...
Notice the above held for $\mu = 0$. However, this choice was arbitrary. We may take advantage of any normal distribution belonging to the location-scale family of the normal distribution, such that we may appropriately transform the above relations of the standard normal to any normal distribution. 

Note then that So while this probability is unique to $\sigma = 2.97$, it holds for any finite $\mu$. So $\sigma$ is unique, but $\mu$ is not. This is because the normal distribution is symmetrical about the mean, i.e. $\Phi(z) = 1 - \Phi(-z)$, $\forall z$. 