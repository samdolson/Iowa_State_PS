...
plot(mu_g, res_g_p,
     xlab = "Fitted (Gamma)", ylab = "Pearson residual",
     main = "Gamma: Pearson Residuals")
abline(h = 0, lty = 2)

plot(mu_ig, res_ig_p,
     xlab = "Fitted (InvGaussian)", ylab = "Pearson residual",
     main = "InvGaussian: Pearson Residuals")
abline(h = 0, lty = 2)
...
The cube-root transformation provides a reasonably linear and variance-stabilizing relationship for the transform-both-sides additive error model, consistent with our earlier findings that cube-root linearization is effective for the GLM as well. Although this approach requires back-transforming predictions to the original CHL scale, it still yields an adequate description of the TN–CHL relationship. However, only means back-transform cleanly; quantities such as variances or percentiles do not, due to issues such as Jensen’s inequality, so inference for non-mean quantities is more complicated under a TBS approach.
...
With MLEs $\hat\theta_0$ and $\hat\theta_1$,

$$
\Lambda = 2\bigl[\ell_1(\hat\theta_1) - \ell_0(\hat\theta_0)\bigr]
$$

which is compared to $\chi^2_2$ because the full model has 5 parameters and the reduced has 3.
