...
=\min\left\{1,\,
\frac{p(\alpha^*,\beta^*\mid y)}{p(\alpha_m,\beta_m\mid y)}\right\}
=\min\left\{1,\,
...
Knowing the data model comes from a one-sample Gamma, we know it comes from the exponential dispersion family. This is used primarily to motivate knowing the form of the sufficient statistics, $S_1 = \sum_{i=1}^n \log y_i$ and $S_2 = \sum_{i=1}^n y_i$. With this, we can write (taking the log of the posterior and then exponentiating at the end to get the original scale): 
...
$\alpha_0 = \frac{\bar{y}^2}{s_{y}^2}\,, \beta_0 = \frac{\bar{y}}{s_{y}^2}$ (for shape-rate parametrization). 

Where $\bar{y}$ and $s_{y}^2$ are the sample mean and variance, respectively (such that $E[Y] = \frac{\alpha}{\beta}$ and $V[Y] = \frac{\alpha}{\beta^2}$). 
...
kable(
  ...
  caption = esc("(MH) Posterior summaries: five-number statistics and 95 central credible intervals")
) |>
...
kable(
  ...
  caption = "(MH) Correlation matrix for alpha and beta"
) |>
  kable_styling(full_width = FALSE, position = "center",
                latex_options = c("hold_position")) |>
  column_spec(1, bold = TRUE)
...
Note: The Credible Intervals are included in Table 2 ("CI. 2.5, CI. 97.5")
...
one_method_tbl <- function(vec_list, method = NULL) {
  ...
  df <- data.frame(Parameter = names(vec_list), round(out, 4), row.names = NULL)
  if (!is.null(method)) df <- cbind(Method = method, df, row.names = NULL)
  df
}

# ---- Final chain (Gibbs) ----
alpha <- final$alpha; beta <- final$beta; mu <- final$mu
gibbs_tbl_final <- one_method_tbl(list(alpha = alpha, beta = beta, mu = mu), method = "Gibbs")
corr_ab <- cor(alpha, beta, use = "complete.obs")

# ---- Optional: a comparison object (MH or another Gibbs) ----
compare_tbl <- NULL
if (exists("compare_obj")) {
  # Set compare label if available; default to "MH"
  compare_label <- if (!is.null(attr(compare_obj, "method"))) attr(compare_obj, "method") else "MH"
  if (is.list(compare_obj) && all(c("alpha","beta","mu") %in% names(compare_obj))) {
    compare_tbl <- one_method_tbl(
      list(alpha = compare_obj$alpha, beta = compare_obj$beta, mu = compare_obj$mu),
      method = compare_label
    )
  }
}

# ---- Combine parameter summaries ----
param_tbl <- if (!is.null(compare_tbl)) rbind(gibbs_tbl_final, compare_tbl) else gibbs_tbl_final

# ---- Full posterior summary (escape % in caption) ----
kable(
  param_tbl, booktabs = TRUE, escape = TRUE,
  caption = "(Gibbs) Posterior summaries: five-number statistics and 95 central credible intervals"
) |>
  kable_styling(full_width = FALSE, position = "center", latex_options = c("hold_position")) |>
  column_spec(1, bold = TRUE) |>
  add_header_above(
    c(" " = if ("Method" %in% names(param_tbl)) 2 else 1,
      "Posterior Summary" = ncol(param_tbl) - if ("Method" %in% names(param_tbl)) 2 else 1)
  )
...
# Accept
# acc_gibbs <- attr(final, "acceptprob")
# 
# if (is.null(acc_gibbs) && is.list(final)) {
#   # common fallbacks used in your objects
#   candidates <- c(
#     "overall_accept","accept_overall","overall","acceptprob",
#     "alpha_accept_overall","beta_accept_overall","mu_accept_overall",
#     "accept_alpha","accept_beta","accept_mu"
#   )
#   hits <- intersect(candidates, names(final))
#   if (length(hits)) acc_gibbs <- unlist(final[hits], use.names = TRUE)
# }
# 
# if (!is.null(acc_gibbs)) {
#   acc_vec <- as.numeric(acc_gibbs)
#   acc_names <- names(acc_gibbs)
#   if (is.null(acc_names) || any(acc_names == "")) {
#     acc_names <- if (length(acc_vec) == 1) "Overall" else paste0("Component_", seq_along(acc_vec))
#   }
# 
#   acc_tbl <- data.frame(
...
#   kable(
#     acc_tbl, booktabs = TRUE, escape = TRUE,
#     caption = "Gibbs acceptance rate (final)"
#   ) |>
#     kable_styling(full_width = FALSE, position = "center",
#                   latex_options = c("hold_position")) |>
#     column_spec(1, bold = TRUE)
# } else {
#   # (Optional) uncomment to see why it's missing during debugging:
#   # print("No acceptance information found on 'final'. Consider storing it in attr(final, 'acceptprob').")
# }
...
kable(
  acc_tbl, booktabs = TRUE, escape = TRUE,
  caption = "(Gibbs) Acceptance rates"
) |>
  kable_styling(full_width = FALSE, position = "center",
                latex_options = c("hold_position")) |>
  column_spec(1, bold = TRUE)
...
# kable(
#   ci_only, booktabs = TRUE, escape = TRUE,
#   caption = "95\\% central credible intervals"
# ) |>
#   kable_styling(full_width = FALSE, position = "center", latex_options = c("hold_position")) |>
#   column_spec(1, bold = TRUE)

# ---- Correlation matrix (alpha, beta) for the final Gibbs chain ----
corr_mat <- matrix(c(1, corr_ab, corr_ab, 1), nrow = 2,
                   dimnames = list(c("alpha","beta"), c("alpha","beta")))
corr_df <- data.frame(Parameter = rownames(corr_mat), round(corr_mat, 4), row.names = NULL)
...
kable(
  corr_df, booktabs = TRUE, escape = TRUE,
  caption = "(Gibbs) Correlation matrix for alpha and beta (Gibbs)"
) |>
  kable_styling(full_width = FALSE, position = "center", latex_options = c("hold_position")) |>
  column_spec(1, bold = TRUE)
...
Note: While the phrasing of the question doesn't explicitly ask for Credible Intervals or 5-number summaries of $\mu$, I felt this might have been an unintentional omission, given they are explicitly asked for previously for the MH method (and these quantities are of interest when comparing methods). 
...
From a more descriptive perspective, the MH posterior histograms appear slightly more “jagged,” reflecting that its random-walk updates move both parameters jointly (possibly yielding higher autocorrelation). By comparison the Gibbs method, which draws $\beta$ directly from its conjugate full conditional and updates $\alpha$ by a Metropolis step, produces smoother histograms and traces. This may also relate to how proposed candidates are treated, as MH proceeds by accepting or rejecting entire candidate pairs, while Gibbs continually updates by drawing directly from conditional distributions, along with their respective acceptance probabilities (16.8% for MH vs. 19.1% for Gibbs).
