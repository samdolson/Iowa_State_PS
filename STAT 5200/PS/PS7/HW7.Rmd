...
Or, explicitly with the given values, the posterior is: 

$$
\boldsymbol{\theta}\mid \mathbf{y}\sim
\text{Dirichlet} (15.10, 50.75, 3.10, 84.75)
$$
...
Recognizing this kernel as a Gamma density, conjugacy gives us posterior(s) of the form: 

$$
\lambda_{ij}\mid y_{ij}\sim \text{Gamma}(\alpha_{ij}+y_{ij},\beta_{ij}+1)
$$

Or, explicitly with the given values, the posterior distributions are: 

$$
\lambda_{11}\mid y_{11}\sim \text{Gamma}(15.5, 6)
$$

$$
\lambda_{10}\mid y_{10}\sim \text{Gamma}(53, 5)
$$

$$
\lambda_{01}\mid y_{01}\sim \text{Gamma}(3.5, 6)
$$

$$
\lambda_{00}\mid y_{00}\sim \text{Gamma}(87, 5)
$$
...
Note: I believe the phrasing of this question is asking for the unnormalized posterior expectations ($E[\lambda_{ij}] = (\alpha_{ij} + y_{ij}) / (\beta_{ij} + 1)$), which were first provided in this section's write up. However, I also provide the normalized quantities $\tilde{\theta}_{ij}$ because I am using those quantities for plug-in RR calculations in Question 3 (and also because it's somewhat unclear which/if both are being explicitly asked for).
...
All three values are close, but not exactly the same. Although we are using the posterior distributions of the components of $RR$ (the $\theta_{ij}$’s or $\lambda_{ij}$’s), the “plug-in” value $RR$ is not the same as the posterior mean $E[RR\mid y]$, which would require integrating the function $RR = g(\boldsymbol{\theta})$ over its full posterior distribution. This difference primarily stems from the nonlinearity of the function $RR$, which means Jensen’s inequality applies: Expectations do not generally “pass through” nonlinear transformations such that they ensure equality.

To be specific, the function $g$ for RR is given by
...
Though this might be repetitive of the above, taking an expectation and then applying a nonlinear function (like this ratio for $RR$) does not yield the same result as applying the function first and then averaging. The difference reflects the variability in the posterior distribution of $\boldsymbol{\theta}\mid \boldsymbol{y}$ (and also the specific nonlinear form of the function $g$). Finally, the observed sample estimate $\widehat{RR}$ also differs slightly because it is based only on the counts, while the Bayesian “plug-in” values incorporate both the data likelihood and prior information.
...
Both posterior distributions of $RR$ place nearly all their (respective) mass(es) above 1 and are "concentrated" (are most dense) in the range of values between 4 to 7. Relative to the sample estimate $\widehat{RR}\approx 6.69$, the "plug-in" estimates show mild shrinkage ($RR(E[\theta])\approx 6.50$ and $RR(\tilde{\theta})\approx 6.04$). 

Most importantly for our interpretation, the posterior probability that $RR>1$ is effectively 1 under both models:

$$
\Pr(RR>1\mid \text{Multinomial–Dirichlet})\approx 0.99996,\qquad
\Pr(RR>1\mid \text{Poisson–Gamma})\approx 0.99990.
$$

Thus, there is very strong posterior evidence that “> 5 years since vaccination” is associated with a higher probability of contracting/"getting" chickenpox in the student population. The 95% credible intervals are somewhat wide on the upper tail (reflecting uncertainty for large $RR$ values) but largely overlap across the two models and include the sample $\widehat{RR}$, indicating consistent conclusions across the two modeling frameworks (Multinomial-Dirichlet and Poisson-Gamma).
