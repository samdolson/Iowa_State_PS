...
&\propto L(\boldsymbol{\theta}\mid \mathbf{y}),p(\boldsymbol{\theta})[3pt]
&\propto
\Big(\prod_{ij}\theta_{ij}^{y_{ij}}\Big)
\Big(\prod_{ij}\theta_{ij}^{\gamma_{ij}-1}\Big)[3pt]
&\propto \prod_{ij}\theta_{ij}^{,{y_{ij}+\gamma_{ij}-1}},
\quad \text{for }\sum_{ij}\theta_{ij}=1
\end{aligned}
$$
...
\times \lambda_{ij}^{\alpha_{ij}-1} e^{-\beta_{ij}\lambda_{ij}}[3pt]
...
$$
\tilde{\theta}{ij}=\frac{E[\lambda{ij}\mid y]}{\sum_{kl}E[\lambda_{kl}\mid y]}
...
The relative risk is generally defined as (Equation 2):
...
For our sample estimate, we have: 
