...
To start, let $i$ denote the sampling site, and let $j$ denote the sampling year, where $i = 1, ..., n_j$ and $j = 1992, ..., 2004$, and where $n_j$ denotes the n-th sampling site on the j-th year. (I believe we could also change the indexing of j to start at 0 or 1, though this decision is arbitrary.)

We may then define the R.V.s as follows: 

Let $Y_{i,j}$ denote the R.V.s for the presence of mayflies at sampling site i on year j, defined as: 
...
We will use the same conventions from part (a), namely: Let $i$ denote the sampling site, and let $j$ denote the sampling year, where $i = 1, ..., n_j$ and $j = 1992, ..., 2004$, and where $n_j$ denotes the n-th sampling site on the j-th year. 

We may then define the R.V.s as follows: 

Let $Y_{i,j}$ denote the R.V.s for the number (abundance) of mayflies at sampling site i on year j, defined as: 
...
*Between years*, independence is plausible because sites are reselected annually, with no sites replicated year-over-year. 
...
Let $i$ denote the sows, where $i = 1, ..., 12$, and $j$ denote the piglets born from sow $i$, where $j = 0, 1, ..., n_i$, where $n_i$ denotes the total number of piglets born from sow $i$. 

We may then define a R.V. for whether piglet $j$ born from sow $i$, was born normal:
...
A R.V. for whether piglet $j$ born from sow $i$, was born weak:
...
A R.V. for whether piglet $j$ born from sow $i$, was stillborn:
