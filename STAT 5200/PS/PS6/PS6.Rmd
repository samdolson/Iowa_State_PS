...
Actual Computation yields (with convergence met) estimates: 
...
```{r, echo = F, warning = F, message = F}
plot(dat$x, dat$y, pch=1, main="Gompertz Growth Model -- Fitted vs. Observed",
     xlab="Time", ylab="Growth")
lines(dat$x, out$yhat, col="steelblue", lwd=2)
...
The `nonlin` function returns the parameter estimates ($\hat{\boldsymbol\theta}=(\hat A,\hat B,\hat k,\hat T)$) and an estimated covariance matrix ($\widehat{\mathrm{Var}}(\hat{\boldsymbol\theta})=\mathbf V$)
...
$$
\hat\theta_j \ \pm\ z_{0.975},\mathrm{SE}(\hat\theta_j)
$$
...
Computing these quantities using the variance covariance matrix calculated in Q2 (and using the `cov2cor` function from the base `stats` package), we have: 
...
Giving us the 6 unique pairwise correlations between $\hat{A}, \hat{B}, \hat{k}, \hat{T}$: 

$$
\text{Corr}(\hat{A}, \hat{B}) = -0.639
$$

$$
\text{Corr}(\hat{A}, \hat{k}) = -0.632
$$

$$
\text{Corr}(\hat{A}, \hat{T}) = -0.029
$$

$$
\text{Corr}(\hat{B}, \hat{k}) = 0.243
$$

$$
\text{Corr}(\hat{B}, \hat{T}) = 0.439
$$

$$
\text{Corr}(\hat{k}, \hat{T}) = 0.104
$$
...
Computing, we get: $\hat k_{\text{abs}} \approx 27.463$, and $\operatorname{SE}(\hat k_{\text{abs}}) \approx 2.220$. 

Taken together, a 90% approximate confidence interval for $k_abs$ then is (23.811, 31.115). 
