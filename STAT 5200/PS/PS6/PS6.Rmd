...
* Growth rate (k): A rather tricky determination. To start, the sharpest rise in the data seems to occur around t = 12 to 18. For example, between t=13 ($y \approx 64)$) and t=17 ($y \approx 140$), the increase is about 76 units over 4 time units, or roughly 19 units per unit time (if we were to fit a "linear" trend over this time period). This provides a sense of the “steepness” of the curve. However, in the Gompertz formulation the parameter (k) is not the slope, but instead, is of the form $\frac{kA}{e}$ (Euler's number, not Euler's constant). Using $A \approx 232$ and equating $\frac{kA}{e}$ with the observed slope ($\approx 19$) gives us a "reasonable" starting estimate of $k \approx 0.22$.
