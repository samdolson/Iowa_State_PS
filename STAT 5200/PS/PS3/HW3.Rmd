...
$\theta_1=\alpha-1,\quad \theta_2=\beta-1, \quad \theta_1 > -1, \quad \theta_2 > -1$.

$T=(T_1,T_2)$ for $T_1(y)=\log(y),\ T_2(y)=\log(1-y)$.

$B(\theta)=-\log\Big(\frac{\Gamma(\theta_1+\theta_2 + 2)}{\Gamma(\theta_1 + 1)\Gamma(\theta_2 + 1)}\Big) = \log \Gamma(\theta_1 + 1) + \log \Gamma(\theta_2 + 1) - \log \Gamma(\theta_1 + \theta_2 + 2)$.
...
$\theta_1= \log(\lambda), \quad \theta_1 \in \mathbb{R}$.
...
$\theta_1 = \alpha - 1 \quad \theta_2 = - \beta, \quad \theta_1 > -1, \quad \theta_2 < 0$

$T = (T_1(y), T_2(y)), \quad T_1(y) = \log (y), \quad T_2(y) = y,$

$B(\mathbf{\theta}) = \log\Gamma(\theta_1 + 1) - (\theta_1 + 1) \log(-\theta_2)$

And 

$c(y) = \mathbb{I}[y >0]$, where $\mathbb{I}$ denotes the indicator function 

Using properties of exponential families, and noting the natural parameters $(\theta_1, \theta_2)$ are linear functions of the parameters $(\alpha, \beta)$, allowing substitution during evaluation (instead of at the end), then: 
...
$$
\phi = \alpha, \quad \theta = -\frac{\beta}{\alpha} \quad \phi > 0, \quad \theta < 0
$$
