...
$$
c(y,\phi)
= \frac{1}{2}\log \phi - \frac{1}{2}\log(2\pi y^{3}) - \frac{\phi}{2y} \quad y>0,\ \phi>0
$$

Note: This parametrization of the exponential dispersion family as given above is not unique.
...
Note: The variance is related to the expected value for this distribution in that it is a function of expectation, i.e., the variance is proportional to the cube of the mean, $\mu$, which corresponds to the expectation as parametrized. 
...
# making datapoints 
# y > 0
y_dat <- seq(0.01, 4, length.out = 5000)

# call function
densities <- lapply(lambdas, function(lam) f_y(y_dat, mu, lam))

# plot
y_max <- max(sapply(densities, max))

# plot
plot(y_dat, densities[[1]], type = "l", col = "red", lwd = 2,
     xlab = "y", 
     ylab = expression(f(y)),
     main = "Density curves",
     ylim = c(0, y_max))
lines(y_dat, densities[[2]], col = "blue", lwd = 2)
lines(y_dat, densities[[3]], col = "green", lwd = 2)
...
So we have $(\alpha, \beta) = (\frac{\lambda}{\mu}, \frac{\lambda}{\mu^2})$ ($\alpha > 0$, $\beta > 0$, and $\mu > 0$, $\lambda > 0$)
...
# data gen
y_dat <- seq(0.01, 3, length.out = 5000)

# densities 
dens_IG <- lapply(lambdas, function(lam) f_invgauss(y_dat, mu, lam))
dens_GA <- lapply(lambdas, function(lam) dgamma(y_dat, shape = lam, rate = lam))

# 1st 
y_max <- max(sapply(c(dens_IG[[1]], dens_GA[[1]]), max))

plot(y_dat, dens_IG[[1]], type = "l", lwd = 2, col = cols[1],
     xlab = "y",
     ylab = expression(f(y)),
     main = "IG (solid) vs. Gamma (dashed) (lambda = 4)",
     ylim = c(0, y_max))

lines(y_dat, dens_GA[[1]], lwd = 2, col = cols[1], lty = 2)
...
# 2nd
y_max <- max(sapply(c(dens_IG[[2]], dens_GA[[2]]), max))

plot(y_dat, dens_IG[[2]], type = "l", lwd = 2, col = cols[2],
     xlab = "y",
     ylab = expression(f(y)),
     main = "IG (solid) vs. Gamma (dashed) (lambda = 8)",
     ylim = c(0, y_max))

lines(y_dat, dens_GA[[2]], lwd = 2, col = cols[2], lty = 2)
...
# 3rd 
y_max <- max(sapply(c(dens_IG[[3]], dens_GA[[3]]), max))

plot(y_dat, dens_IG[[3]], type = "l", lwd = 2, col = cols[3],
     xlab = "y",
     ylab = expression(f(y)),
     main = "IG (solid) vs. Gamma (dashed) (lambda = 16)",
     ylim = c(0, y_max))

lines(y_dat, dens_GA[[3]], lwd = 2, col = cols[3], lty = 2)
...
Yes, there appear to be systematic differences between the inverse Gaussian and Gamma distributions despite when they share the same mean and variance. The Gamma distribution is consistently less "peaked" and has different "tail behavior" of their respective distributions than the inverse Gaussian. I believe this is because of differences in higher-order moments (skewness and kurtosis). Generally it seems that, holding $\mu$ constant, as $\lambda$ increases, both distributions have greater density about the mean, and the discrepancy between them diminishes, i.e., they more closely "overlap" one another; but the inverse Gaussian remains more sharply peaked with heavier tails compared to the Gamma with the same mean and variance.

Note: The systematic differences above reflect differences in **distributional form** beyond what is summarized by skewness or kurtosis. The two families differ because their variance–mean relationships have different forms:

$$
\text{V}(Y) \propto \mu^3 \quad \text{(For the Inverse-Gaussian)}
$$

$$
\text{V}(Y) \propto \mu^2 \quad \text{(For the Gamma)}
$$

This difference between the two variance functions drives the observed shape differences seen above, i.e., the differences in the "tail behavior" of the distributions, which skewness and kurtosis alone cannot fully characterize. 
