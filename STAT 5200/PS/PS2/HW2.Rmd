...
# data gen
y_dat <- seq(0.01, 5, length.out = 5000)
...
plot(y_dat, dens_IG[[1]], type = "l", lwd = 2, col = cols[1],
     xlab = "y",
     ylab = expression(f(y)),
     main = "IG (solid) vs. Gamma (dashed) (Full Range)",
     ylim = c(0, y_max))

# IG curves
lines(y_dat, dens_IG[[2]], lwd = 2, col = cols[2])
lines(y_dat, dens_IG[[3]], lwd = 2, col = cols[3])
...
plot(y_dat, dens_IG[[1]], type = "l", lwd = 2, col = cols[1],
     xlab = "y",
     ylab = expression(f(y)),
     main = "IG (solid) vs. Gamma (dashed) (Right-Half)",
     ylim = c(0, y_max))
...
```{r, echo = F}
y_dat <- seq(0.00001, 1, length.out = 5000)
...
plot(y_dat, dens_IG[[1]], type = "l", lwd = 2, col = cols[1],
     xlab = "y",
     ylab = expression(f(y)),
     main = "IG (solid) vs. Gamma (dashed) (Left-Half)",
     ylim = c(0, y_max))
...
This distinction between variance functions drives the observed shape differences seen above, i.e., the differences in the "tail behavior" of the distributions, which skewness and kurtosis alone cannot fully account for. 
