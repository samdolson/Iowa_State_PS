...
# Reference Notes
# rows are replicates
# columns are lake-specific
# dat original observations
# mcmc1 :: tau, lambda, x1, x2 simulations
# mcmc2 :: beta, sigma, alpha simulations

# Read-in data
dat <- read.table("LMBdat_for601.txt", header = TRUE)
mcmc1 <- read.table("LMBMCMC1.txt", header = TRUE)
mcmc2 <- as.matrix(read.table("LMBMCMC2.txt", header = TRUE))
# Objects for later
n_lakes <- 75
...
# Posterior draws from LMBMCMC2.txt 
beta_s <- mcmc2[, 1:n_lakes, drop = FALSE]
sig2_s <- mcmc2[, (n_lakes + 1):(2*n_lakes), drop = FALSE]
alpha_s <- mcmc2[, 2*n_lakes + 1]
...
q1_tbl <- data.frame(
  lake_id = 1:n_lakes,
  lknum = lake_levels,
  post_mean_mu15 = mu15_lake_means,
  lb90 = mu15_ci90[, 1],
  ub90 = mu15_ci90[, 2]
)
...
# standardize 
z <- (w0 - mu15) / sd15
pr_exceed_draws <- 1 - pnorm(z)
# aggregate by lake
pr_exceed_mean <- colMeans(pr_exceed_draws)

q2_tbl <- data.frame(
  lake_id = 1:n_lakes,
  lknum = lake_levels,
  post_mean_pr_exceed = pr_exceed_mean
)

# order small to large (again)
# this also re-indexes the lakes (again)
q2_tbl_ord <- q2_tbl[order(q2_tbl$post_mean_pr_exceed), ]
q2_tbl_ord$rank <- seq_len(nrow(q2_tbl_ord))

# plot
par(mar = c(4, 6, 2, 2))
plot(x = q2_tbl_ord$post_mean_pr_exceed, 
     y = q2_tbl_ord$rank,
