...
f(x)
&=
\prod_{i=1}^{n_1}\phi(y_{1,i};\mu_1,\sigma_1^2)\,
\prod_{i=1}^{n_2}\phi(y_{2,i};\mu_2,\sigma_2^2) \\[6pt]
&= 
\prod_{i=1}^{n_1} \frac{1}{\sqrt{2\pi\sigma_1^2}} \exp\!\left\{ -\frac{(y_{1,i}-\mu_1)^2}{2\sigma_1^2} \right\} 
\prod_{i=1}^{n_2} \frac{1}{\sqrt{2\pi\sigma_2^2}} \exp\!\left\{ -\frac{(y_{2,i}-\mu_2)^2}{2\sigma_2^2} \right\} \\[6pt]
&=
(2\pi)^{-(n_1+n_2)/2}
(\sigma_1^2)^{-n_1/2}
(\sigma_2^2)^{-n_2/2}
\exp\left(
-\frac{1}{2\sigma_1^2}\sum_{i=1}^{n_1}(y_{1,i}-\mu_1)^2
-\frac{1}{2\sigma_2^2}\sum_{i=1}^{n_2}(y_{2,i}-\mu_2)^2
\right), 
\end{aligned}
$$
...
The approximate confidence interval for $\mu_1-\mu_2$ is
...
Under model $M_1$, $\bar Y_1$ and $\bar Y_2$ are independent normal random variables, and hence $\bar Y_1-\bar Y_2$ is normally distributed. The interval (11.14) is an approximate confidence interval, justified by the asymptotic normality of the centered studentized statistic

$$
\frac{(\bar Y_1-\bar Y_2)-(\mu_1-\mu_2)}
{\left(\dfrac{S_1^2}{n_1}+\dfrac{S_2^2}{n_2}\right)^{1/2}},
$$

which is asymptotically standard normal.

Given this, define the function
...
where $\mathbf{1}\{\cdot\}$ denotes the indicator function. Thus $g(X)\in\{0,1\}$ indicates whether the approximate confidence interval covers the true parameter value $\mu_1-\mu_2$.

The exact coverage probability of the interval under model $M_1$ is
...
Let $X_1^*,\ldots,X_M^* \stackrel{iid}{\sim} f$ be independent Monte Carlo samples generated under model $M_1$. The Monte Carlo approximation to this coverage probability is the sample average
...
for a chain of length M, block length b, and $k = M - b + 1$. Further, note: $I(A)$ is the identity function that assumes a value of 1 if A is true, and 0 otherwise, and $L_{M,b}(y)$ is the empirical CDF of the subsampling values
...
These conditions match the target equal-tail requirements from Hint (b) and confirm that the subsampling confidence interval is given by
