...
Pots are the experimental units in this experiment because the genotype "treatments" (1 or 2) were randomly assigned to pots. Because there are multiple observations per experimental unit, it is important to explicitly include a random effect for each experimental unit in the model.
...
This is a test of a whole-plot (helmet type) effect. The appropriate test statistic uses the variation among helmet types divided by the variation among helmets within each type:

$$
t = \sqrt{\frac{MS_{\text{Type}}}{MS_{\text{Helmet(Type)}}}} = \sqrt{\frac{226/1}{254/8}} \approx \sqrt{7.11} \approx 2.67
$$

This statistic follows a noncentral t-distribution with 8 degrees of freedom.
...
From the model, the variance of the difference in type means is:

$$
\text{Var}(\bar{y}_{1\cdots} - \bar{y}_{2\cdots}) = \frac{1}{10} (4\sigma_a^2 + 2\sigma_b^2 + \sigma_e^2)
$$
...
This follows directly from the expected mean squares and the structure of the linear mixed model.
...
We estimate $\sigma_a^2$ by equating expectations of the mean squares:

$$
\hat{\sigma}_a^2 = \frac{MS_{\text{Helmet(Type)}} - MS_{\text{Dir} \times \text{Helmet(Type)}}}{4} = \frac{254/8 - 114/8}{4} = \frac{140}{32} = 4.375
$$

This uses the expectation difference of variance components associated with whole-plot and subplot levels.
...
We are given the BLUE estimate is 0.5. The variance for this estimator is:
...
$$
\hat{\text{Var}} = \frac{1}{5} \left(\frac{114}{8}\right) = 2.85, \quad t_{0.975,8} = 2.306
$$

The 95% confidence interval is:

$$
0.5 \pm 2.306 \cdot \sqrt{2.85} = 0.5 \pm 2.306 \cdot 1.688 = ( -3.39, 4.39 )
$$

where $t_{0.975,8} = 2.306$ and the degrees of freedom (8) come from $Direction \times Helmet(Type)$ used previously.
...
We estimate the standard error for the BLUE of the difference between high and low intensity impacts in the front direction for type 1 helmets:
...
$$
\hat{\text{Var}} = \frac{1}{5} \left( \frac{114}{8} + \frac{59}{16} \right) = \frac{1}{5} \cdot \frac{287}{16} = 3.5875
$$
