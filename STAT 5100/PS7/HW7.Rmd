...
Based on the model assumptions of $e_{ij} \overset{iid}{\sim} N(0, \sigma_e^2)$, for each subject $j = 1, \ldots, 20$,
...
$$
\begin{aligned}
E(d_j) &= \mu_1 - \mu_2, \quad \text{Var}(d_j) = \text{Var}(e_{1j}) + \text{Var}(e_{2j}) = 2\sigma_e^2 \\
\text{Cov}(d_j, d_{j'}) &= \text{Cov}(e_{1j} - e_{2j}, e_{1j'} - e_{2j'}) = 0 \quad \text{for } j \ne j'
\end{aligned}
$$

Thus, $d_j \overset{iid}{\sim} N(\mu_1 - \mu_2, 2\sigma_e^2)$, which is a constant mean model:

$$
\boldsymbol{d} = \mathbf{1}[\mu_1 - \mu_2] + \boldsymbol{\epsilon}, \quad \boldsymbol{\epsilon} \sim N(0, 2\sigma_e^2 \mathbf{I})
$$
...
Let $\bar{d} = \frac{1}{20} \sum_{j=1}^{20} d_j$. Then $\bar{d} \sim N(\mu_d, \frac{\sigma_d^2}{20})$, where $\mu_d = \mu_1 - \mu_2$, $\sigma_d^2 = 2\sigma_e^2$.
...
$$
F \sim F_{1,19} \left( \frac{5(\mu_1 - \mu_2)^2}{\sigma_e^2} \right)
$$
