...
$$
\text{Var}(d_j) = \text{Var}(e_{1j}) + \text{Var}(e_{2j}) = 2\sigma_e^2
$$
...
Thus, $d_j \overset{iid}{\sim} N(\mu_1 - \mu_2, 2\sigma_e^2)$,

Where: 

$$
\boldsymbol{d} = \mathbf{1_{20 \times 1}}[\mu_1 - \mu_2] + \boldsymbol{\epsilon}, \quad \boldsymbol{\epsilon} \sim N(0, 2\sigma_e^2 \mathbf{I})
$$

And we can take advantage of describing the differences within the framework of a GMM. 
...
Let $\bar{d} = \frac{1}{20} \sum_{j=1}^{20} d_j$. 

Then $\bar{d} \sim N(\mu_d, \frac{\sigma_d^2}{20})$, 

Where $\mu_d = \mu_1 - \mu_2$, $\sigma_d^2 = 2\sigma_e^2$.

Under $H_0: \mu_d = 0$, the test statistic is given by:
...
$$
t \sim t_{19} \left( \frac{\mu_d}{\sqrt{\sigma_d^2/20}} \right) \overset{d}{=} t_{19} \left( \frac{\mu_1 - \mu_2}{\sqrt{\sigma_e^2 / 10}} \right)
$$

And: 

$$
F \sim F_{1,19} \left( \frac{5(\mu_1 - \mu_2)^2}{\sigma_e^2} \right) \overset{d}{=} F_{1,19} \left( \frac{10(\mu_1 - \mu_2)^2}{\sigma_d^2} \right)
$$
...
The model for all scores is given by:

$$
\mathbf{y} = \left[ \mathbf{I}_{2\times2} \otimes \mathbf{1}_{20\times1} \right] 
\begin{bmatrix}
\mu_1 \\
\mu_2
\end{bmatrix}
...
Where:
  ...
  - $a_j$ and $b_j$ be scores from groups 1 and 2
  - $\bar{a} = \frac{1}{20} \sum a_j$, $\bar{b} = \frac{1}{20} \sum b_j$
...
$$
\hat{\text{MSE}} = \frac{1}{380} \left( \sum_{j=1}^{20}(a_j - \bar{a})^2 + \sum_{j=1}^{20}(b_j - \bar{b})^2 \right)
$$
...
$$
\hat{\sigma}_d^2 = 2\hat{\sigma}_e^2 = \frac{1}{19} \sum_{j=1}^{20}(d_j - \bar{d})^2
$$

From part d), we have:

$$
\sigma_u^2 + \sigma_e^2 = \frac{1}{38} \left( \sum_{j=1}^{20}(a_j - \bar{a})^2 + \sum_{j=1}^{20}(b_j - \bar{b})^2 \right)
$$

We can treat this as a system of linear equations, such that we may solve for $\hat{\sigma}_e^2$ and $\hat{\sigma}_u^2$:

$$
\hat{\sigma}_e^2 = \frac{1}{38} \sum_{j=1}^{20} (d_j - \bar{d})^2
$$

And: 

$$
\hat{\sigma}_u^2 = \frac{1}{38} \left( \sum_{j=1}^{20}(a_j - \bar{a})^2 + \sum_{j=1}^{20}(b_j - \bar{b})^2 \right) - \hat{\sigma}_e^2
$$
...
Both $\bar{d}$ and $\bar{a} - \bar{b}$ are independent unbiased estimators. So:

$$
\mu_1 - \mu_2 = \frac{\text{Var}^{-1}(\bar{d})}{\text{Var}^{-1}(\bar{d}) + \text{Var}^{-1}(\bar{a} - \bar{b})} \cdot \bar{d} + \frac{\text{Var}^{-1}(\bar{a} - \bar{b})}{\text{Var}^{-1}(\bar{d}) + \text{Var}^{-1}(\bar{a} - \bar{b})} \cdot (\bar{a} - \bar{b})
...
The first condition is multi-faceted, but I would say falls under the perview of the "Suppose..." from Slide 2 of the Chapter 12 Slides. That is, we assume: 
...
This ensures that the normal equations have a unique solution for $\boldsymbol{\beta}$, and thus both the GLS and OLS estimators are uniquely defined.

Without full column rank, $(\mathbf{X}^\top \mathbf{X})^{-1}$ does not exist, and we would need to use a generalized inverse to define the estimators, in which case the equivalence in (2) may not hold.

#### 3. 

For estimable $\mathbf{C}\mathbf{\beta}$, $\mathbf{C}\hat{\mathbf{\beta}}_{\hat{\mathbf{\Sigma}}} = \mathbf{C}\hat{\mathbf{\beta}}$
...
Meaning the inverse of the variance-covariance matrix $\boldsymbol{\Sigma}^{-1}$ acts like the identity on the column space of $\mathbf{X}$, even if $\boldsymbol{\Sigma}$ is not a scalar multiple of the identity on the full space.
...
Assume that the assumptions from part a) all hold. 
...
$$
\hat{\mathbf{\beta}}_{GLS} = (\mathbf{X}^\top \mathbf{\Sigma}^{-1} \mathbf{X})^{-1} \mathbf{X}^\top \mathbf{\Sigma}^{-1} \mathbf{y}
$$
...
We may treat $\frac{1}{\sigma^2}$ as a scalar for the purposes of matrix algebra, such that we have:
...
Noting that $(aA)^{-1} = \frac{1}{a} A^{-1}$, applying to the above and simplifying:

$$
\hat{\boldsymbol{\beta}}_{\boldsymbol{\Sigma}} = \sigma^2 \left( \mathbf{X}^\top \mathbf{X} \right)^{-1} \cdot \frac{1}{\sigma^2} \mathbf{X}^\top \mathbf{y}
= \left( \mathbf{X}^\top \mathbf{X} \right)^{-1} \mathbf{X}^\top \mathbf{y}
= \hat{\boldsymbol{\beta}}_{\text{OLS}}
$$

Noting that by definition, noting the normal equations, OLS is given by: 

$$
\hat{\boldsymbol{\beta}}_{\text{OLS}} = \left( \mathbf{X}^\top \mathbf{X} \right)^{-1} \mathbf{X}^\top \mathbf{y}
$$

So, the primary condition required for the above to work is for $\boldsymbol{\Sigma} = \sigma^2 \mathbf{I}$. 