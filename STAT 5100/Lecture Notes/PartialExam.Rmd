...
The definition of estimability is given above as first defined in Chapter 2. 
...
There does **not** exist a matrix \( A \) such that:

$$
C = A \mathbf{X}
$$

Hence, \( \alpha_1 \) is **not estimable** (this is not in row space of $\mathbf{X}$ and is not estimable). 

Using the above definition, no, $\alpha_1$ is not estimable. 

Parameters are only estimable if they can be expressed as a linear combination of the rows of the design matrix. Additionally, the model matrix does not have full rank, which limits what is estimable in this scenario.  
...
Then, the corresponding matrix \( A \) (a row vector in \( \mathbb{R}^{1 \times 4} \)) satisfying \( C = A \mathbf{X} \) is:
...
This is exactly the first row of \( \mathbf{X} \), so:
...
Noting the "Prerequisite Knowledge" Slides and an extension of the OLS Slides used for the following. 
...
The higher the column rank of $\mathbf{X}$, the more parameters in $\boldsymbol{\beta}$ that are estimable.
