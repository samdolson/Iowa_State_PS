...
Q1 b, c: Do we need to include an interpretation, even though it doesn't explicitly ask for one? 
...
Overall, we observe that statistical tests are consistent across the residual types (original, sqrt, and log transformed), in that in all instances of Shapiro-Wilk we do not reject the null hypothesis that the (original/sqrt/log) residuals are normally distributed. The same is roughly true when reviewing a visual assessment of either the histogram or of the associated QQ Plots. Overall, all three forms of our response (and residual) appear to be normally distributed. 

Taken together, we overall decide to move forward with the square root transformation, as it provides the most solid evidence that none of our key assumptions for our model are being violated. 
