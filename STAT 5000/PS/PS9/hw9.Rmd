...
num <- sum(6 * (temperature - temperatureMean)*(sample_mean - responseMean))
...
```{r}
n_i <- 6
SS_xx <- sum((temperature - temperatureMean)^2)
RSS <- sum((n_i - 1) * sample_variance + n_i * (sample_mean - (b0 + b1 * temperature))^2)
s_squared <- RSS / (length(temperature) * n_i - 2)

SEb1 <- sqrt(s_squared / SS_xx)
SEb0 <- sqrt(s_squared * (1 / (length(temperature) * n_i) + (temperatureMean^2) / SS_xx))
...
|---------------------|--------------------|----------------|-------------|
| Regression on X     | 1                  | 2436.0000      | 2436.000000 |
| Residuals           | 22                 | 2468.158       | 112.189     |
| - Lack-of-fit       | 2                  | 2446.158       | 1223.079	  |
...
# because expected value of regression is expected value of response 
SSR <- sum(n_i * (sample_mean - responseMean)^2)  

SSLOF <- sum(n_i * (sample_mean - (responseMean + (sample_mean - responseMean) * (temperature - temperatureMean) / SS_xx))^2)
SSPE <- sum((n_i - 1) * sample_variances)
...
# sources <- c("Regression on X", "Residuals", "Lack-of-fit", "Pure error", "Total")
# dfs <- c(df_regression, df_residual, df_lack_of_fit, df_pure_error, df_total)
# SS <- c(SSR, SSE, SSLOF, SSPE, SST)
# MSE <-  c(MSR, MSE, MSLOF, MSPE)
...
$$
F = \frac{{MS_{\text{Lack of Fit}}}}{MS_{\text{Pure Error}}} = \frac{1223.079}{1.100000} \approx 1111.89
$$
For comparison with the F Statistic, we have: 
Lack of Fit Df = 2
Pure Error Df = 20

So, the computed F-statistic for the lack-of-fit test is 1111.89 with degrees of freedom (2, 20). Given the provided p-value of of 0.0001 is very small (much less the typical $\alpha = 0.05$), we have overwhelming evidence to reject the null hypothesis that the simple linear regression model is adequate (fits the relationship well) for the data provided. 
