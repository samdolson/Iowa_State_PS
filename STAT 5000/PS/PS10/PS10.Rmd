...
correlations <- sapply(explanatory_vars, function(var) {
  cor(data[[var]], data[[response_var]])
})
print(correlations)

correlation_results <- sapply(explanatory_vars, function(var) {
...
1. HT2:
   - Correlation: $0.0426$
   - $p$-value: $0.7264$
   - Not Significant: The correlation is very weak and the p-value indicates no evidence to reject the null hypothesis.

2. HT9:
   - Correlation: $0.2369$
   - $p$-value: $0.0483$
   - Significant: The correlation is moderate, and the p-value is below 0.05, indicating evidence of a relationship between HT9 and BMI at age 18.

3. WT2:
   - Correlation: $0.1909$
   - $p$-value: $0.1133$
   - Not Significant: While the correlation is moderate, the p-value is above 0.05, suggesting insufficient evidence of a relationship.

4. WT9:
   - Correlation: $0.5459$
   - $p$-value: $1.018837 \times 10^{-6}$
   - Significant: The correlation is strong, and the extremely low p-value indicates a highly significant relationship between WT9 and BMI at age 18.

5. ST9:
   - Correlation: $0.0056$
   - $p$-value: $0.9633$
   - Not Significant: The correlation is negligible, and the p-value indicates no evidence of a relationship.

The explanatory variables HT9 and WT9 have statistically significant correlations with BMI at age 18 ($p < 0.05$). Among these, WT9 shows the strongest relationship. The other variables ($\text{HT2}, \text{WT2}, \text{ST9}$) do not have significant correlations.
...
```{r}
# Subset the data for the explanatory variables
explanatory_vars <- c("HT2", "HT9", "WT2", "WT9", "ST9")
...
# Compute the p-values for correlations
n <- nrow(explanatory_data) # Sample size
...
# Remove duplicates (Var1 == Var2 or reversed pairs)
...
The table lists pairs of variables with significant correlations ($p < 0.05$). Below is a summary of the relationships:

1. Variables with significant correlations with $\text{HT2}$:
   - $\text{HT9}$ ($r = 0.7384$, $p \approx 3.00 \times 10^{-13}$): Strong positive correlation.
   - $\text{WT2}$ ($r = 0.6445$, $p \approx 1.73 \times 10^{-9}$): Strong positive correlation.
   - $\text{WT9}$ ($r = 0.5229$, $p \approx 3.42 \times 10^{-6}$): Moderate positive correlation.
   - $\text{ST9}$ ($r = 0.3617$, $p \approx 2.09 \times 10^{-3}$): Weak positive correlation.

2. Variables with significant correlations with $\text{HT9}$:
   - $\text{HT2}$ ($r = 0.7384$, $p \approx 3.00 \times 10^{-13}$): Strong positive correlation.
   - $\text{WT2}$ ($r = 0.6071$, $p \approx 2.52 \times 10^{-8}$): Strong positive correlation.
   - $\text{WT9}$ ($r = 0.7276$, $p \approx 9.69 \times 10^{-13}$): Strong positive correlation.
   - $\text{ST9}$ ($r = 0.6034$, $p \approx 3.23 \times 10^{-8}$): Strong positive correlation.

3. Variables with significant correlations with $\text{WT2}$:
   - $\text{HT2}$ ($r = 0.6445$, $p \approx 1.73 \times 10^{-9}$): Strong positive correlation.
   - $\text{HT9}$ ($r = 0.6071$, $p \approx 2.52 \times 10^{-8}$): Strong positive correlation.
   - $\text{WT9}$ ($r = 0.6925$, $p \approx 3.11 \times 10^{-11}$): Strong positive correlation.
   - $\text{ST9}$ ($r = 0.4516$, $p \approx 8.71 \times 10^{-5}$): Moderate positive correlation.

4. Variables with significant correlations with $\text{WT9}$:
   - $\text{HT2}$ ($r = 0.5229$, $p \approx 3.42 \times 10^{-6}$): Moderate positive correlation.
   - $\text{HT9}$ ($r = 0.7276$, $p \approx 9.69 \times 10^{-13}$): Strong positive correlation.
   - $\text{WT2}$ ($r = 0.6925$, $p \approx 3.11 \times 10^{-11}$): Strong positive correlation.
   - $\text{ST9}$ ($r = 0.4530$, $p \approx 8.22 \times 10^{-5}$): Moderate positive correlation.

5. Variables with significant correlations with $\text{ST9}$:
   - $\text{HT2}$ ($r = 0.3617$, $p \approx 2.09 \times 10^{-3}$): Weak positive correlation.
   - $\text{HT9}$ ($r = 0.6034$, $p \approx 3.23 \times 10^{-8}$): Strong positive correlation.
   - $\text{WT2}$ ($r = 0.4516$, $p \approx 8.71 \times 10^{-5}$): Moderate positive correlation.
   - $\text{WT9}$ ($r = 0.4530$, $p \approx 8.22 \times 10^{-5}$): Moderate positive correlation.

- $\text{HT9}, \text{WT9}, \text{WT2}$, and $\text{ST9}$ are strongly or moderately correlated with multiple other variables.
- These relationships are important to consider when building models to avoid issues like multicollinearity.
...
Overall: 
- Strength at age 9 ($\text{ST9}$) does not significantly predict BMI at age 18, as indicated by the non-significant slope ($p = 0.963$).
- The residual plots do not reveal major violations of assumptions, but there is a potential outlier that may influence the analysis.
- Overall, this model is not suitable for explaining the variability in BMI at age 18. Alternative predictors should be explored to better understand BMI at age 18.
...
1. Estimate of $\beta_1$ (Effect of Strength at Age 9):
...
2. Is the Effect of Strength at Age 9 Significant?
...
Conclusion:
1. The estimate of $\beta_1$ (effect of $\text{ST9}$) in the multiple regression model differs from the simple regression model due to adjustment for $\text{WT9}$. This is expected because $\text{WT9}$ and $\text{ST9}$ are correlated.
2. In the multiple regression model, the effect of $\text{ST9}$ is significant ($p = 0.00668$), indicating that strength at age 9 contributes uniquely to predicting BMI at age 18, after accounting for weight at age 9.
3. Adding $\text{WT9}$ significantly improves the model fit, as shown by the increase in $R^2$.
...
$R^2$ Value:
  - $R^2 = 0.4431$:
  - This indicates that 44.31% of the variability in BMI at age 18 is explained by the model, which includes weight and height at ages 2 and 9 ($\text{WT2}, \text{HT2}, \text{WT9}, \text{HT9}$), and strength at age 9 ($\text{ST9}$).

The model explains a moderate portion of the variation in BMI at age 18 using early-life measurements of weight, height, and strength. This suggests that these predictors collectively have a meaningful, though not complete, ability to account for differences in BMI at age 18 among the individuals in the dataset. While other factors not included in the model likely contribute to the remaining unexplained variability (approximately 55.69%), the predictors used provide useful insights into BMI development from early and middle childhood characteristics.
...
From the provided regression output for the model:
...
In the context of the multiple linear regression model predicting Sale Price ($Y$) from Living Area ($X_1$) and Age ($X_2$), the parameters $\beta$ have the following interpretations:

1. Intercept ($\beta_0$):  
   The intercept represents the expected Sale Price of a house when both Living Area ($X_1$) and Age ($X_2$) are 0. From the output, $\beta_0 = 66,464$. This value is only meaningful if 0 is within the range of the observed data for both predictors.

2. Living Area ($\beta_1$):  
   The parameter $\beta_1 = 102.62349$ represents the expected change in Sale Price for each additional square foot of Living Area, holding Age constant. This suggests that, on average, a one-square-foot increase in Living Area is associated with an increase of approximately $102.62 in Sale Price.

3. Age ($\beta_2$):  
   The parameter $\beta_2 = -1072.88363$ represents the expected change in Sale Price for each additional year of Age, holding Living Area constant. This indicates that, on average, each additional year in the house's age is associated with a decrease of approximately $1,072.88 in Sale Price.

### Context
These coefficients quantify the individual contributions of Living Area and Age to the Sale Price of homes in Ames, Iowa, while controlling for the other variable in the model.

## (b) 

What is the value of $R^2$and its interpretation for the model including Living Area and Age?

An $R^2$of 0.6865 means that approximately 68.65% of the variability in Sale Price is explained by the linear regression model that includes Living Area and Age as predictors.
...
From the output, the p-value is reported as $< 0.0001$. This indicates extremely strong evidence against the null hypothesis.

Given the extremely small p-value ($< 0.0001$), we reject the null hypothesis at any reasonable significance level (e.g., $\alpha = 0.05$).

The F-test indicates that the regression model including Living Area and Age is statistically significant overall. This means that the predictors (Living Area and Age) collectively explain a significant proportion of the variability in Sale Price. In other words, the model has significant explanatory power.
...
$$
t = \frac{\hat{\beta}_1}{\text{SE}} = \frac{102.62349}{1.74084} \approx 58.95
$$

From the output, the p-value for Living Area is reported as $< 0.0001$, indicating strong evidence against the null hypothesis.

Since the p-value is much smaller than any reasonable significance level (e.g., $\alpha = 0.05$), we reject the null hypothesis.
...
$$
t = \frac{\hat{\beta}_2}{\text{SE}} = \frac{-1072.88363}{28.21248} \approx -38.03
$$

From the output, the p-value for Age is reported as $< 0.0001$, indicating strong evidence against the null hypothesis.
...
To calculate the reduction in the Sum of Squares for Error ($\text{SSE}$) due to adding Basement Area and Total Room to the model, we compare the SSE from the initial model (with Living Area and Age only) to the updated model (with Living Area, Age, Basement Area, and Total Room).
...
$$
\text{Reduction in SSE} = \text{SSE (initial model)} - \text{SSE (updated model)}
$$

$$
\text{Reduction in SSE} = 5.85885 \times 10^{12} - 4.36077 \times 10^{12}
$$

$$
\text{Reduction in SSE} = 1.49808 \times 10^{12}
$$

By adding Basement Area and Total Room to the model, the Sum of Squares for Error was reduced by $1.49808 \times 10^{12}$, indicating that these two variables contribute significantly to explaining the variation in Sale Price.
...
$$
F = \frac{\left( \frac{\text{Reduction in SSE}}{\text{DF}_{\text{new}} - \text{DF}_{\text{old}}} \right)}{\text{MSE (updated model)}}
$$
...
$$
F = \frac{\left( \frac{1.49808 \times 10^{12}}{2} \right)}{1493926329}
$$

$$
F = \frac{7.4904 \times 10^{11}}{1493926329} \approx 5014.58
$$

Using the output and the extremely large F-value of $5014.58$, the p-value is reported as $< 0.0001$. This indicates strong evidence against the null hypothesis.
