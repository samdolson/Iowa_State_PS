...
fancyDiff <- function(data, categoryCol, meanCol) {
  ...
  mean1 <- mean(as.function.as.on(character2)[as.function(character1)=="Control"])
...
thing <- function(df, meanCol, categoryCol, categoryVal) {
  x1 <- df %>%
    as.data.frame %>% 
    select(as.character(meanCol), as.character(categoryCol))  
  
  x2 <- x1 %>% 
    filter(paste0(categoryCol) == as.character(categoryVal))  %>% 
    nrow
...
thing(guinea_pigs, "Time", "Treatment", "Control")
nrow(controlData)
```
