...
The p-value from the permutation test is 0.0597. However, this again is a two-tailed p-value. We must again divide this by two to obtain the one-tailed p-value, which is as follows. 

$\text{p-value (one-tailed)} = 0.0597/2 = 0.02985$ 

Alternatively, we can directly look at the number of samples in the randomization test where the obtained difference in the means is for the particular side of the distribution we're interested in, given from the count noted above and a corresponding (one-tailed) p-value of: 
$p-value = \frac{279}{10,000} = 0.0279$
