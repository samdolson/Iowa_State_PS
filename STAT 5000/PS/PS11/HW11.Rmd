...
This model has has the best Mallow's Cp value overall. Additionally, this model has the second best Adjusted $R^2$ compared to all models (and the best for a model with 3 explanatory variables). In comparison to the best adjusted R-squared, the difference is only 0.0029 less than the first best. 
...
Increasing Garage Size to "Large" is associated with an increased Sales Price of $15,833 compared to a Garage Size of "Small", all else (all other explanatory variables) being equal.
...
$$
\widehat{\text{Sale Price}} = 32873 + 58.128 * \text{Basement Area} + 104.75703 * \text{Living Area} - 8481.53784 * \text{Total Room} - 746.48764 * \text{Age}
$$
...
$$
\text{MSE}_{\text{train}} = 1.5777 \times 10^{9}
$$

$$
\text{MSE}_{\text{eval}} = 1.61699 \times 10^{9}
$$

$$
\text{Difference in MSE} = 3.43 \times 10^{7}
$$
...
$$
\text{Relative Difference} = \frac{\text{Difference in MSE}}{\text{MSE}_{\text{eval}}} * 100 = \frac{3.43 \times 10^7}{1.61699 * 10^9} \times 100 \approx 2.12\%
$$

Or a roughly 2.12 percent increase in MSE when going to the eval dataset. 

This tells us that: When the MSE (Train) is slightly smaller than the MSE (Eval), meaning the model we chose performs marginally better on the training dataset than the evaluation dataset. However, because the relative difference isn't especially extreme, I believe this isn't all bad news! Generally this marginal relative difference indicates that our model is not overfitting, and that it is generalizing well from the training to the evalution dataset. It is performing "ok". 
...
Of note, all explanatory variables except for "z" meet the significance threshold of "overwhelming" or "strong" evidence (in favor of rejecting the null hypothesis). 
...
Consistent with part (d), of note, all explanatory variables except for "z" meet the significance threshold of "overwhelming" or "strong" evidence (in favor of rejecting the null hypothesis). 
