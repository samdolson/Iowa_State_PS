...
$\beta_0$: mean arm span (cm) when height (cm) is 0. Yes, this is not especially realistic (arm span at 0 height) 

$\beta_1$: the increase in mean arm span (cm) when height (cm) increases by 1 cm
...
b1 is the model-estimated value of the parameter $\beta_1$, also know as the estimated increase in mean arm span (cm) when height (cm) increases by 1 cm
...
F = 428.4 with p-value <2e-16, provides overwhelming evidence to reject the null hypothesis (can also say this meets the $\alpha = 0.05$ level significance), such that we have evidence that there is a significant linear relationship between arm span (in cm) and height (in cm). 
...
Our conclusion is that using Y = Arm Span (cm) as the response, the data are consistent with the straight line model using height (cm) as our one response, specifically:
