...
# Q3 
Do you calculate the actual cell means or estimate them based on the code provided?
...
```{r}
model <- lm(protein ~ copper + zinc + copper*zinc, data = minnows)
summary(model)
```
...
F-value: 1.809
p-value: 0.227
Interpretation: The p-value of 0.227 does not meet our threshold of the $\alpha = 0.05$ level, such that we do not have evidence to reject the null hypothesis that there is no significant difference in the mean protein content ($\mu$g/tank) between the two copper levels (0 ppm and 150 ppm), when averaging across levels of zinc concentrations. 
...
F-value: 1.113
p-value: 0.388
Interpretation: The p-value of 0.388 does not meet our threshold of the $\alpha = 0.05$ level, such that we do not have evidence to reject the null hypothesis that the the mean protein content ($\mu$g/tank) of interactions is different across the pairs of interactions between copper and zinc concentrations. 
...
Overall, all three residual plots do not appear to have cause for concern, though there is some slight evidence/indication that we do not have constant variance in our residuals. 
