...
rates <- data.frame(
  Estimator = c(
    ...
    "Histogram (1D)",
    "KDE (1D)",
    "NW / Local Linear Regression (1D)",
    "Multivariate KDE / LPR (d-dim)",
    "Deconvolution (Ordinary Smooth)",
    "Deconvolution (Supersmooth)"
  ),
  Bias = c(
    "$O(n^{-1})$",
    "$O(h)$",
  ...
  Variance = c(
    ...
    "$O((nh)^{-1})$",
    "$O((n h^d)^{-1})$",
    "$\\text{inflated by }\\lvert\\Psi_e(t)\\rvert^{-2}$",
    "$\\exp(c / h^{\\beta})$"
  ),
  Optimal_Bandwidth = c(
    "None",
...
knitr::kable(
  rates,
  escape = FALSE,
  caption = "Summary of Rates of Convergence for Common Estimators (Optimal Rate is MSE/MISE)"
)
```
