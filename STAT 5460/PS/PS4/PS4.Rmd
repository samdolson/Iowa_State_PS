...
Let $(X_1, Y_1), \cdots (X_n, Y_n)$ be an independent identically distributed sample from $(X,Y)$, with the typical model of the form: 

$$
Y_i = m(X_i) + \sigma(X_i)e_i,\qquad \mathbb{E}[e_i]=0,\quad \mathrm{Var}(e_i)=1
$$

and $X$ and $e$ are independent. Also, for my own sanity, note: $\mathbb{X} = (X_1, \cdots, X_n)$. 
...
Let $\mathbf{X}$ be the $n \times (p+1)$ design matrix with $(j+1)$ column $(X_i-x_0)^j$.
...
Note: There are two different moment vectors in this derivation: $c_n = (S_{n,p+1},\ldots,S_{n,2p+1})^\top$ and $c_p = (\mu_{p+1},\ldots,\mu_{2p+1})^\top$. The text does also uses both, but only explicitly defines $c_n$. Further, $c_n$ denotes the sample vector of weighted empirical moments while $c_p$ is the population moment vector appearing in the asymptotic bias formula.

Continuing on, for continuous $f_X$ at $x_0$, 

$$ 
S_n \approx n f_X(x_0)\, H S H, \qquad c_n \approx n f_X(x_0)\, H c_p\, h^{p+1}
$$ 
...
\qquad
\mathbf{X}^\top \mathbf{W}^2 \mathbf{X} \approx \frac{n f_X(x_0)}{h}\, H\mathbf{S}^{\star}H
$$
...
Using $\mathrm{Var}(Y_i\mid\mathbb{X})\approx\sigma^2(x_0)$ and assumption of independence (specifically independence of errors conditional on $\mathbb{X}$),
...
$$
K^{\star}_{\nu,p}(u) = \varepsilon_{\nu+1}^\top \mathbf{S}^{-1} v_p(u)\,K(u) \tag{4.27, but using u instead of t}
$$
...
$$ 
x_i = \begin{pmatrix} 1 & (X_i - x_0) & \cdots & (X_i - x_0)^p \end{pmatrix}^\top, \qquad X = \begin{pmatrix} x_1^\top \ \vdots \ x_n^\top \end{pmatrix} 
$$
