...
The order-$p$ local polynomial at $x_0$ minimizes the objective function of the form (and assuming that the (p+1)th derivative of the regression function $m$ at point $x_0$ exists):
...
We then turn back to our objective function (an estimator of $m^{(\nu)}(x_0)$): 

$$
\hat{m}^{(\nu)}(x_0) = \sum_{i=1}^n\left(Y_i-\sum_{j=0}^p \beta_j (X_i-x_0)^j\right)^2 K_h(X_i-x_0)
$$
...
Let $S_{n,j}=\sum_{i=1}^n (X_i-x_0)^j K_h(X_i-x_0)$ and $c_n=(S_{n,p+1},\ldots,S_{n,2p+1})^\top$. 
...
Substitute into the prior equation to get:
...
For $p-\nu$ odd, the leading term $\mathbf{c}_p$ does not cancel, so we're left with: 
...
Define the equivalent kernel functions: 

$$
K^{\star}_{\nu,p}(t)=\varepsilon_{\nu+1}^\top \mathbf{S}^{-1}\left( 1, t, \cdots, t^{p} \right) ^{\top}K(t)
$$
...
This kernel satisfies the moment conditions:
...
Substitute into the previous equation to get:
...
Define the equivalent kernel functions as before: 
...
Note: This result also relies upon WLS representation and variance in addition to the results noted in the book.   
