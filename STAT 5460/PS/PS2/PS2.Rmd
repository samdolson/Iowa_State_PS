...
header-includes:
  ...
  - \fancyhead[R]{\LARGE STAT 5460 HW II (Technically III?)}
  - \fancyfoot[C]{\thepage}
...
&= \text E\left[K_h(x-X)\right] \quad\text{X's i.i.d., specifically identical}\ \\
...
Notably, as X's are i.i.d., then the Z's are i.i.d., as defined.
...
\quad\text{(sum of the variance of i.i.d. R.V.'s)} \\ 
...
I do not believe the other two integrals evaluate/simplify nicely, and thus will be left to a form of simplification more akin to notational convenience. We then have the overall (exact) MISE is of the form: 
...
for some $\epsilon$, and show how the bound depends on $n, h, \epsilon$ and $| K |_\infty = \sup_{u \in \text{R}} |K(u)| < \infty$.
...
Note: If we assume the kernel is a valid density, i.e., non-negative, the answer changes somewhat to: 

$$
0 \le Y_i \le \frac{|K|_\infty}{h}
$$
...
P\left( \big|\hat f_h(x) - \text{E}[\hat f_h(x)]\big| > \epsilon \right)
\le
2\exp\left( - \frac{2 n h^2 \epsilon^2}{|K|_\infty^2} \right)
$$
...
(1): As $n \to \infty$, $h \to 0$ with $n h^2 \to \infty$.

(2): $X_1,\dots,X_n$ are i.i.d. with density $f$ (ensures we can apply Hoeffding’s inequality).

(3): $K$ is bounded, $|K|_\infty = \sup_{u\in\mathbb{R}} |K(u)| < \infty$.

(4): $K$ is Lipshitz continuous (For a somewhat stronger assumption, we could instead say $K$ is differentiable with bounded derivative, $| K'|_\infty = \sup_{u\in\mathbb{R}} |K'(u)| < \infty$).

Note: The stronger version of Condition 4 implies the kernel density estimator (and its expectation) is Lipschitz continuous on the compact set $[a,b]$. This Lipschitz assumption let us reduce from a supremum over a (possibly) infinite set to a maximum over a finite net.

(5): There is a note at the end of the problem for the additional assumption that K is a valid density, i.e., non-negative. This assumption is not required for the overall proof to work as-is, but is included to show how we may get some "improvement", similar to the ending note in part a). Overall, this assumption is not required and not called upon until after the proof using assumptions 1 through 4. 
...
$$ 
|Y_i(x) - Y_i(x')| \le \frac{|K'|_{\infty}}{h^2}\,|x - x'|
\;\;\Rightarrow\;\;
|\hat f_h(x) - \hat f_h(x')|
= \Bigg|\frac{1}{n}\sum_{i=1}^n\!\big(Y_i(x)-Y_i(x')\big)\Bigg|
...
$$
\big|\hat f_h(x) - \hat f_h(x')\big| \le \frac{|K'|_{\infty}}{h^2}\,|x - x'|
$$
...
Fix $\delta > 0$. Construct a $\delta$-net $N_\delta \subset [a,b]$ so that
...
For any $x \in [a,b]$ and its nearby grid point $x' \in N_\delta$,
...
Where the first and last terms are bounded using the Lipschitz condition.

(Note: The additional terms come from "adding zeros" via $\pm \hat{f}_{h}(x') \pm \text{E}\hat{f}_{h}(x')$, followed by the Triangle Inequality)

Choose

$$ 
\delta = \frac{\epsilon h^{2}}{4|K'|_{\infty}} \quad\Rightarrow\quad \frac{2|K'|_{\infty}}{h^{2}}\,\delta = \frac{\epsilon}{2} 
$$

Then

$$ 
\Big\{ \sup_{x\in[a,b]} \big|\hat f_h(x)-\text{E}\hat f_h(x)\big|>\epsilon \Big\} \subseteq \Big\{ \max_{x'\in N_\delta} \big|\hat f_h(x')-\text{E}\hat f_h(x')\big|>\tfrac{\epsilon}{2} \Big\} 
$$

By the union bound,

$$ 
\text{P}\!\left( \sup_{x\in[a,b]} \big|\hat f_h(x)-\text{E}\hat f_h(x)\big|>\epsilon \right) \le |N_\delta|\, \max_{x'\in N_\delta} \text{P}\!\left(\big|\hat f_h(x')-\text{E}\hat f_h(x')\big|>\tfrac{\epsilon}{2}\right)
$$

From part a), Hoeffding’s inequality gives for each $x'$:

$$
\text{P}\left(\big|\hat f_h(x') - \text{E}\hat f_h(x')\big| > \tfrac{\epsilon}{2}\right)
\le 2\exp\left(-\frac{n h^2 \epsilon^2}{8 | K|_\infty^2}\right)
$$

$$
\text{P}\left( \sup_{x\in[a,b]} \big|\hat f_h(x) - \text{E}\hat f_h(x)\big| > \epsilon \right)
\le \left( \left\lceil \frac{4(b-a)| K'|_\infty}{\epsilon h^2} \right\rceil + 1 \right)
\cdot 2\exp\left(-\frac{n h^2 \epsilon^2}{8 | K|_\infty^2}\right)
$$

We then need to determine whether this term is "something small". To that end note that from the bound
...
Assuming K is a valid density then, we have a modified second term on the right-side of the inequality, of the form: 

$$
\text{P}\left( \sup_{x\in[a,b]} \big|\hat f_h(x) - \text{E}\hat f_h(x)\big| > \epsilon \right)
\le \left( \left\lceil \frac{4(b-a)| K'|_\infty}{\epsilon h^2} \right\rceil + 1 \right)
\cdot 2\exp\left(-\frac{n h^2 \epsilon^2}{2 | K|_\infty^2}\right)
$$

I.e., 

$$
2\exp\left(-\frac{n h^2 \epsilon^2}{8 | K|_\infty^2}\right) \Rightarrow 2\exp\left(-\frac{n h^2 \epsilon^2}{2 | K|_\infty^2}\right)
$$

Which is still "something small", and given the negative exponent converges to 0 faster than before when we don't make the assumption K is a valid density. 
...
$$
L(x)=\hat f_h(x)-\big(t_\alpha(\delta)+2\Lambda\delta\big), \quad 
U(x)=\hat f_h(x)+\big(t_\alpha(\delta)+2\Lambda\delta\big)
$$

(Using $\Lambda$ and $t_\alpha(\delta)$ as defined previously.)

Note: If we assume the kernel is a valid density, i.e., non-negative, the answer changes somewhat to: 

$$
t_{\alpha, \text{old}}(\delta)=\sqrt{\frac{8\,|K|_{\infty}^2}{n\,h^2}\,
\log\!\Big(\frac{2\,m_\delta}{\alpha}\Big)}
\Rightarrow 
t_{\alpha, \text{new}}(\delta)=\sqrt{\frac{2\,|K|_{\infty}^2}{n\,h^2}\,
\log\!\Big(\frac{2\,m_\delta}{\alpha}\Big)}
$$

So, effectively, the form of the lower and upper confidence bounds remain the same, we just change the $t_\alpha(\delta)$ based on the bounds derived in parts a). and b). 