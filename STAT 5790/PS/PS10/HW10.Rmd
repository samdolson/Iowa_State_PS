...
```{r}
psiFunction <- function(params, yIjk, te, tr) {
  rho <- params[1]
  t1 <- params[2]
  t2 <- params[3]
...
estimateParameters <- function(yIj, te, tr) {
  ...
  t1Matrix <- matrix(4.0, nrow, ncol) # Default value
  t2Matrix <- matrix(0.001, nrow, ncol) # Default value
  ...
  for (i in 1:nrow) {
    for (j in 1:ncol) {
      ...
      if (sum(yIjk > 0) <= 2) {
        ...
        # Debugging: Check yIjk and te/tr before optimization
        if (any(is.nan(yIjk)) || any(is.infinite(yIjk))) {
          cat("Invalid yIjk for pixel (", i, ",", j, ")\n")
        }
