...
library(tidyverse)
library(dplyr)
result1 <- analysisData %>%
  group_by(billType) %>%
  summarize(
      zero = sum(Arlen.Specter..PA. == 0, na.rm = TRUE), 
      one = sum(Arlen.Specter..PA. == 1, na.rm = TRUE), 
      against = sum(Arlen.Specter..PA. == -1, na.rm = TRUE)
  )

# result2 <- analysisData %>%
#   group_by(billType) %>%
#   summarize(
#       zero = sum(analysisData[,2] == 0, na.rm = TRUE), 
#       one = sum(analysisData[,2] == 1, na.rm = TRUE), 
#       against = sum(analysisData[,2] == -1, na.rm = TRUE)
#   )
...
senatorData <- function(data) {
  x <- as.character(colnames(analysisData[data]))
