...
```{r}
set.seed(50)
UniformNums <- rep(runif(2, min = 0, max = 1), 1000)
matrixDf <- matrix(UniformNums, nrow  = 2, ncol = 1000)
dim(matrixDf)
```

## (b)    
Calculate the mean *X* =[1]{.underline} *n* *n* *i*=1*Xi* for each   sample. (Hint: consider using matrix operations.) *6 points*    
...
## (b) 
Using for instance the function table, cross-classify the passengers by gender and passenger class. Do a further
cross-tabulation, perhaps using the same function additionally stratified by survival status. Comment on your findings. *3 + 3 + 3 points*
...
## (e) 
or each bill type identified earlier, tabulate the average number of times senators voted with, against, or indifferently from the Senate majority leader. *10 points*
