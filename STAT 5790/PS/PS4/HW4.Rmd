...
euclidean_distances <- array(0, dim = c(22, 810, 20))  # Initialize distances array

# Loop through 22 rows and 810 columns
for (gene_idx in 1:22) {
  for (sample_idx in 1:810) {
    for (mean_idx in 1:20) {
      euclidean_distances[gene_idx, sample_idx, mean_idx] <- sqrt(sum((mean_abundance[gene_idx, sample_idx] - micromeans_matrix[mean_idx, ])^2))
    }
  }
}

# Step 4: Find the closest mean for each gene and sample
closest_means_indices <- apply(euclidean_distances, c(1, 2), which.min)

# Step 5: Tabulate the frequency distribution
frequency_distribution <- table(closest_means_indices)

# Step 6: Display the frequencies using a pie chart
