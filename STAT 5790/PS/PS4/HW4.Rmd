...
deviationMat2 <- ziptrain

deviationMatFill <- do.call(rbind, lapply(X = 0:9,
                           FUN = function(x) {
                             meanDigit <- matrix(meanImages[, x + 1], nrow = 16, ncol = 16)
                             deviationMat2[zipdigit == x, ] <-
                               deviationMat2[zipdigit == x, ] - as.vector(t(meanDigit))
                             }
                           )
              )

# svdDf <- svd(x1)
# svd_result <- svd(deviationMatFill)

svd_result <- svd(deviationMat)
svdDf <- svd(deviationMatFill)
...
for (k in k_values) {
  ...
  for (digit in 0:9) {
    meanDigit <- matrix(meanImages[, digit + 1], nrow = 16, ncol = 16)  # Reshape to 16x16
...
#     meanDigit <- matrix(meanImages[, x + 1], nrow = 16, ncol = 16)  # Reshape to 16x16
