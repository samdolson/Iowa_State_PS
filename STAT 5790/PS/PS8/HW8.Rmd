...
polaroid <- function(x) {
  ...
  # If R is numerically zero, return just the radius
  if (abs(R) < .Machine$double.eps) return(c(R))
  
  # Compute theta1 in the range [0, 2 * pi]
  theta1 <- atan2(x[2], x[1])
  if (theta1 < 0) theta1 <- theta1 + 2 * pi
  
  p <- length(x)
  
  # Only calculate additional angles if there are more than two dimensions
  if (p > 2) {
    numerators <- sqrt(rev(cumsum(rev(x[-c(1, 2)]^2))))
    denominators <- sqrt(cumsum(rev(x[-1]^2))[-1])
    
    # Ensure numerators and denominators are of the same length
    if (length(numerators) != length(denominators)) {
      warning("Length mismatch: Check the input vector for inconsistencies.")
      return(NA)
    }
    
    # Compute cosine values and restrict them to the range [-1, 1]
    cos_values <- numerators / denominators
    cos_values <- pmin(pmax(cos_values, -1), 1)
